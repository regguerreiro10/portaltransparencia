...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT a.id, a.nome, a.arquivo, a.licitacao_id
        FROM licitacao_anexo a
        INNER JOIN licitacao l ON l.id = a.licitacao_id
        WHERE a.id = :id
        LIMIT 1
    ");
    $stmt->execute([':id' => $id]);
    ...
    if (!is_file($filePath)) {
        throw new RuntimeException('Arquivo do edital nao localizado.');
    }

    $update = $pdo->prepare("UPDATE licitacao SET downloads = downloads + 1 WHERE id = :id");
    $update->execute([':id' => (int) $attachment['licitacao_id']]);
