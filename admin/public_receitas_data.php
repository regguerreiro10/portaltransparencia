...
try {
    ...
    $periodSql = "
        SELECT DISTINCT DATE_FORMAT(COALESCE(c.dt_pagamento, c.dt_emissao, c.dt_vencimento), '%Y-%m') AS period
        FROM conta c
        WHERE c.tipo_conta_id = :tipo_conta
          AND COALESCE(c.dt_pagamento, c.dt_emissao, c.dt_vencimento) IS NOT NULL
        ORDER BY period DESC
    ";

    $periodStmt = $pdo->prepare($periodSql);
    $periodStmt->execute([':tipo_conta' => 1]);
    ...
    if ($period !== '') {
        $where[] = "DATE_FORMAT(COALESCE(c.dt_pagamento, c.dt_emissao, c.dt_vencimento), '%Y-%m') = :period";
    ...
    $dataSql = "
        SELECT
            ...
            COALESCE(p.nome, 'Sem cliente') AS cliente,
    ...
    echo json_encode([
        ...
        'links' => [
            'list' => 'admin/index.php?class=ContaReceberList',
            'create' => 'admin/index.php?class=ContaReceberForm',
        ],
